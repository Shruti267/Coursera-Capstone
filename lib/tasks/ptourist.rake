  ...
  def create_thing thing, organizer, members, images, types=[]
    ...
    types.each do |type|
      if type[:id].nil?
        ...
        type=Type.create(:creator_id=>organizer.id,:name=>type[:name])
      ...
      ThingType.new(:thing=>thing, :type=>type,
                    :creator_id=>organizer.id)
               .tap {|tt| tt.is_vip_pass_required=true if type[:is_vip_pass_required]}.save!
    end
  end
    ...
    types=[
      {
       ...
       :notes=>"Free train rides are available on the hour for VIP pass holders.",
